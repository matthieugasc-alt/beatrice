...
def append_log(timestamp_iso, company, document_type, filename, sha256, status, directory, original_path="", pennylane_status="", accounting_supplier=""):
    rows = read_log_rows()
...
def analyze_pdf_content(file_path):
    ...
    if (
        "loyer" in text_lower
        or "provision pour charges" in text_lower
        or "gestionnaire locatif" in text_lower
        or "bail" in text_lower
    ):
        return supplier_clean, "Loyer"

    # Signal fort hôtel
    if "hotel" in text_lower or "hôtel" in text_lower:
        return supplier_clean, "Hotel"
    ...
    return supplier_clean, accounting_category
...
def beatrice():
    ...
    if period != "all":
        ...
        for row in rows:
            ...
            elif period == "month" and ts.year == now.year and ts.month == now.month:
                filtered.append(row)
    ...
    total_factures = sum(1 for row in rows if row.get("document_type") == "facture")
    total_devis = sum(1 for row in rows if row.get("document_type") == "devis")
    total_autres = sum(1 for row in rows if row.get("document_type") == "autre")
    total_doublons = sum(1 for row in rows if row.get("status") == "duplicate")
    total_pending = sum(1 for row in rows if row.get("pennylane_status") == "pending")
    total_sent = sum(1 for row in rows if row.get("pennylane_status") == "sent")
    total_non_traites = sum(
        1 for row in rows
        if row.get("status") == "saved" and row.get("pennylane_status") == ""
    )
    ...
    for row in rows:
        ...
        accounting_supplier = escape(row.get("accounting_supplier", ""))
    ...
    <tr>
        <td colspan="9">Aucun document pour le moment.</td>
    </tr>
    """
...
<div class="actions">
<form method="get" action="/beatrice" style="margin-bottom:15px;">
    <input type="date" name="start">
    <input type="date" name="end">
    <button type="submit">Filtrer</button>
</form>
    <a class="button" href="/beatrice">Tout</a>
    <a class="button" href="/beatrice?period=today">Aujourd'hui</a>
    <a class="button" href="/beatrice?period=month">Ce mois</a>
    <a class="button" href="/beatrice?period=year">Cette année</a>
    <a class="button" href="/beatrice/anomalies">Anomalies</a>
</div>
        <div class="kpis">
            <div class="kpi"><strong>{total_documents}</strong><br>Total</div>
            <div class="kpi"><strong>{total_factures}</strong><br>Factures</div>
            <div class="kpi"><strong>{total_devis}</strong><br>Devis</div>
            <div class="kpi"><strong>{total_autres}</strong><br>Autres</div>
            <div class="kpi"><strong>{total_doublons}</strong><br>Doublons</div>
            <div class="kpi"><strong>{total_pending}</strong><br>Pending</div>
            <div class="kpi"><strong>{total_sent}</strong><br>Sent</div>
            <div class="kpi"><strong>{total_non_traites}</strong><br>Non traités</div>
        </div>
        <div class="actions">
...
def beatrice_anomalies():
    ...
    body = "".join(html_rows) if html_rows else """
    <tr>
        <td colspan="9">Aucune anomalie pour le moment.</td>
    </tr>
    """
...
def pennylane_pending():
    ...
    for row in rows:
        ...
        pending.append({
            ...
            "accounting_supplier": row.get("accounting_supplier", ""),
            "directory": row.get("directory", ""),
...
def upload():
    ...
    accounting_supplier = get_accounting_supplier(f.filename, document_type)
    ...
    if file_hash in hash_index:
        ...
        real_supplier, accounting_category = analyze_pdf_content(path)
        ...
        append_log(
            ...
            pennylane_status="",
            accounting_supplier=accounting_category
        )
